...
function WorkspaceTrigger({
	...
	if (renderTrigger) {
		return (
			<PopoverTrigger render={<button className={className} {...props} />}>{renderTrigger(selectedWorkspace, open)}</PopoverTrigger>
		);
	}

	return (
		<PopoverTrigger render={<button data-state={open ? 'open' : 'closed'} className={cn(
        					...
        					className,
        				)} {...props} />}><div className="flex min-w-0 flex-1 items-center gap-2">
        				...
        				</div><ChevronsUpDownIcon className="h-4 w-4 shrink-0 opacity-50" /></PopoverTrigger>
	);
}
