...
export function WashFoldPricingCard({ channel }: { channel: WashFoldChannel }) {
  const { t } = useLanguage();
  const p = WASH_FOLD_PRICING[channel];

  const sameDayNoteKey =
    channel === 'instore' ? 'pricingPage.instore.sameDay.note' : 'pricingPage.delivery.sameDay.note';

  return (
    <Card className="flex min-w-0 flex-col">
      <div
        className="flex flex-wrap items-center justify-between gap-4 border-b border-gray-200 bg-[#00bfb3]/10 px-6 py-7 sm:flex-nowrap"
      >
      ...
      <div>
        <div className="flex flex-col justify-between gap-2 border-b border-gray-200 px-6 py-3.5 sm:flex-row sm:items-center sm:gap-4">
          <div>
            ...
            <div className="text-sm text-gray-500">{t(sameDayNoteKey)}</div>
          </div>
